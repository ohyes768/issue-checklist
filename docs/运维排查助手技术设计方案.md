...
### 2026-02-11 - Docker 容器化部署
**变更类型**: 新增部署方案

**主要变更内容**:
1. **Docker 容器化**:
   - 新增后端容器: Dockerfile.backend (Python 3.11 + FastAPI + Uvicorn)
   - 新增前端容器: Dockerfile.frontend (React + Vite + Nginx 多阶段构建)
   - 新增 Docker Compose 编排: docker-compose.yml
   - 新增 Nginx 反向代理配置: docker/nginx.conf

2. **跨平台部署方案**:
   - Windows 本地构建 → 镜像导出 → Linux 服务器部署
   - docker buildx 多平台构建支持 (AMD64/ARM64)
   - 部署脚本: scripts/export-images.bat, scripts/load-images.sh

3. **前端环境配置优化**:
   - 新增 .env.production 生产环境变量
   - vite.config.ts 添加开发环境 API 代理
   - api.ts 优化 API 基础 URL 配置逻辑

4. **部署文档**:
   - 新增 docker-deployment.md - 本地 Docker 部署指南
   - 新增 docker-linux-deployment.md - Windows 打包 + Linux 部署指南

**影响范围**:
- Dockerfile.backend, Dockerfile.frontend - 新增容器镜像构建配置
- docker-compose.yml - 容器编排配置
- docker/nginx.conf - Nginx 反向代理配置
- web-frontend/.env.production - 生产环境变量
- web-frontend/vite.config.ts - 开发 API 代理
- web-frontend/src/app/services/api.ts - API URL 配置优化
- requirements.txt - 添加 FastAPI 相关依赖

**用户体验变化**:
- 支持一键 Docker 容器化部署
- 支持 Windows 本地打包、Linux 服务器部署
- 前端生产环境自动通过 Nginx 代理访问后端
- 数据持久化，支持 YAML 文件热更新
...
**跨平台部署方案**:
- **方案一**: Windows 本地构建镜像 → 导出 tar → Linux 服务器加载
- **方案二**: 使用 docker buildx 构建多平台镜像（AMD64/ARM64）

**数据持久化**:
- `./data:/app/data` - YAML 数据文件挂载，支持热更新
- `./logs:/app/logs` - 日志文件持久化存储

**环境配置**:
- 开发环境: 前端使用 `VITE_API_BASE_URL` 指定后端地址
- 生产环境: 前端使用相对路径，通过 Nginx 代理到后端

...
- **UI改进**:
  - 优化首页卡片布局，添加检查单数量显示
  - 卡片样式更紧凑，调整布局：检查单数量（左）| 优先级（右）
  - 移除不必要的图标，简化界面
- **问题详情页重构**:
  - 新增独立的"相关知识库"模块，集中展示所有wiki/gif/script链接
  - How to Check 和 Fix Steps 只保留描述文本
  - 简化链接命名：wiki1、wiki2... / gif1、gif2... / 脚本1、脚本2...
- **换行显示修复**:
  - 前端添加智能格式化函数，自动识别编号列表并换行
  - 解决JSON序列化后换行符转义丢失的问题
- **影响范围**:
  - 首页问题卡片显示优化
  - 问题详情页结构重组
  - 后端API添加checklistCount字段

### 2026-01-22 - 初始版本 (Streamlit)
